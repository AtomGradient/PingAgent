...
- **Harness participant mode**: A Harness-launched participant keeps using `ai-ping <to>` and `--reply-to <delivery-id>`, but the intent returns to Host through a Host-issued scoped context. Host authenticates the sender with the local IPC peer PID and the exact owned descendant process chain. This mode rejects `--from`, never writes or falls back to a legacy mailbox, and leaves waiting/querying to the Host delivery read model instead of legacy `--wait`.
